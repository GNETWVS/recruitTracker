function [keepMask,rejectMask,addedMask,stats,closeFlag,skipFlag] = rT_processCoralMask(stats,keepMask,...
...
for kk = 1:numel(stats)
    ...
    mask = rT_makeMaskFromStats(stats(kk),s);
    % color code them for the user
    rT_displayUpdatedImage(Idisplay,labelMap,keepMask,rejectMask,keepColor,rejectColor,mask,processed,maskColor,processedColor);
    ...
    if k == 0
        ...
        elseif logical(pEdit.UserData)
            newMask = rT_fineTuneBlobBoundaries(Ioriginal,resizeFactor,mask);
        ...
        elseif logical(pAutoOff.UserData)
            ...
            clear stats;
            labelMap = rT_labelCoralsInImage(keepMask,1,resizeFactor);
        ...
        if k == 0
            if logical(pDraw.UserData)
                ...
                if sum(newMask(:)) ~=0
                    ...
                    % update the legends
                    labelMap = rT_labelCoralsInImage(keepMask,stats,resizeFactor);
                    rT_displayUpdatedImage(Idisplay,labelMap,keepMask,rejectMask,keepColor,rejectColor);
