...
function Ibw = rT_adaptiveThresholding2(coralMask,Ism,cmThreshold,pixToCM,resizeFactor)
...
sumI = zeros(s(1),s(2));
for i = 1:numel(ths)
    curI = rT_adaptivethresh_PK(G,M,ths(i),'gaussian','relative');
...
for i = 1:nmax
    sumI = Ibw > i;
    sumI = rT_filterCoralsbySizeInImage(sumI,cmThreshold,pixToCM);
...
out = rT_filterCoralsbySizeInImage(out,cmThreshold,pixToCM);
